...
- Checks: `ref-path`, `ref-command`, `ref-target`, `size-bytes`, `shape-headings`, `shape-lines` (headings and list items), and `shape-body` (other lines, off by default). The rest of the plan's ids are registered as planned so configs can name them. `ref-path` skips git refs such as `origin/main`.
