...
- Checks: `ref-path`, `ref-command`, `ref-target`, `fact-version`, `fact-layout`, `fact-tool`, `drift-copies`, `drift-nested`, `drift-stale`, `drift-age`, `git-untracked`, `git-local-tracked`, `size-bytes`, `shape-headings`, `shape-lines` (headings and list items), and `shape-body` (other lines, off by default). The rest of the plan's ids are registered as planned so configs can name them. `ref-path` skips git refs such as `origin/main`. `ref-command` accepts a program the repo declares by config file or toolchain.
- `.herdr-llm-lint.toml` with `files`, `size_bytes`, `line_chars`, `age_commits`, `disable`, `enable`, `fail_on`, and `[llm]`.
